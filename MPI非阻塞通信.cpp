...
//	{
	...
	if (rank == 0) {	
		...
		for (int j = 1; j < mpisize; j++) {
            ...
            {
                MPI_Request request;
                MPI_Isend(&m1[i][0], n, MPI_FLOAT, j, i, MPI_COMM_WORLD, &request);
            }
        }
	}
	else 
    {
		...
		for(int i=rank;i<n;i+=mpisize-1)
			MPI_Irecv(&m1[i][0], n, MPI_FLOAT,0, i, MPI_COMM_WORLD, &request);	
...
/*            {
            ...
            {
                MPI_Request request;
				MPI_Isend(&m1[k][0], n, MPI_FLOAT, j,n+k + 1, MPI_COMM_WORLD, &request);
            }
		}
		else
        {
            MPI_Request request;
            MPI_Irecv(&m1[k][0], n, MPI_FLOAT, 0, n+k + 1, MPI_COMM_WORLD, &request); //&status);
		...
		if (rank != 0) {    
            ...
            for (int i = r2; i < n; i += mpisize - 1) 
            {
				...
				m1[i][k] = 0;
				if (i == k + 1) 
					MPI_Isend(&m1[i][0], n, MPI_FLOAT, 0, 0, MPI_COMM_WORLD, &request);	
			}
...
// 			}
        ...
        {
			MPI_Irecv(&m1[k + 1][0], n, MPI_FLOAT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &request);
            MPI_Wait(&request, MPI_STATUSES_IGNORE);
        }
	}
	MPI_Barrier(MPI_COMM_WORLD);
